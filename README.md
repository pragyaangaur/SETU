...
A row is written before its outcome exists, so it cannot be selected afterwards. Rows that were replayed through the hours the feed still held when the service first started are marked as backfilled and counted separately, because a forecast made from a record that already exists is a weaker claim than one written down in advance.

Rows older than two days move into [docs/data/verification.json](docs/data/verification.json), which is a daily count that is added to and never rewritten. Once a day is closed its numbers are final, so a bad week cannot be quietly dropped later.
...
Four gaps in this project can only be closed by people outside it. A request for each one was sent on 5 September 2026, to an address read off the holding institution's own website rather than to a guess. The letters themselves are held privately rather than in this repository. One has been answered, and the answer is recorded in the fourth item below.

1. **Ground magnetometer data for Shillong and Tirunelveli**, held by the Indian Institute of Geomagnetism. This is the largest gap. The model trains on Alibag and Hyderabad, near ten degrees geomagnetic latitude, and is applied to a region near sixteen.
2. **Magnetotelluric resistivity profiles** across the Shillong Plateau and the Brahmaputra valley, to replace the four ground models assembled from published literature. The geoelectric field scales with the square root of ground resistivity, so this assumption drives the answer more than any parameter in the power network does. The survey that covers this ground was done by the Indian Institute of Geomagnetism rather than by the institute usually assumed, so this request and the one above go in the same letter.
3. **Whether any transformer neutral current monitoring exists** anywhere in the region. There is no public measurement of neutral current in India, which is why every consequence figure here rests on physics and on measurements made in other countries.
4. **The latency of the Aditya-L1 solar wind products.** India has its own monitor at the first Lagrange point and a warning system for the Indian grid should be reading it. The data itself is already public, so the question worth asking was how long it takes to arrive, because the whole warning time is the travel time from the spacecraft and any delay comes straight out of it.

    **This one has been answered.** The Aditya-L1 Support Cell replied on 6 September 2026 that it takes at least 48 hours for Aditya-L1 data to become available. The travel time this system forecasts into is 25 to 80 minutes, so a 48 hour archive cannot feed a warning system at all. ASPEX and MAG are science instruments and their archive is built for science, where 48 hours is quick. A low latency operational product is separate engineering and it does not exist yet. Until it does, an Indian warning system for the Indian grid has to read a foreign spacecraft, and that is the finding rather than a complaint.
...
The forecast model is trained on Alibag and Hyderabad, which sit near ten degrees geomagnetic latitude, and applied to a region near sixteen. Shillong and Tirunelveli are held by the Indian Institute of Geomagnetism, and closing that gap needs a data request to the institute.
...
There is no real time Aditya-L1 solar wind product. The Aditya-L1 Support Cell confirmed on 6 September 2026 that the data takes at least 48 hours to become available, which is roughly a thousand times the warning window this system works in. The chain is therefore designed against that interface and demonstrated on the operational NOAA feed and the OMNI archive, and it is ready for an Indian low latency feed on the day one exists.
